...
| 結果 | 怎麼做 |
|------|--------|
| `chezmoi 未安裝，略過此步` | 這台電腦沒裝，略過，照常往下做，別為此中斷開工 |
| 落差 A、B 都乾淨 | 一致，直接往下做，**不用回報這一步**（別拿雜訊佔開工報告） |
| 任一段有落差 | **停下來**，改跑 `chezmoi-sync` 的完整流程（步驟 3、4），照它的報告格式給使用者，等點頭再動手。**開工在此暫停**，處理完才回來繼續 |
...
1. **開工是「讀」、收工是「寫」**——本技能自己只讀、只報告，不改任何檔案。唯一例外是步驟 0 的 dotfile 處置：那是委派給 `chezmoi-sync`，由它照自己的規矩問過使用者才動手，不由本技能決定
