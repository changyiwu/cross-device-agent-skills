...
- 組路徑一律 `Join-Path`，不要出現字面 `\`（為什麼會靜默失敗見下面第 5 坑）
- 正則裡的分隔符一律 `[\\/]`
- 需要單一分隔符字元時用 `[IO.Path]::DirectorySeparatorChar`

## 五個坑（都是「不報錯」型的）

這五個的共同特徵：**失敗時沒有任何錯誤訊息**，所以要先知道才找得到。
...
### 5. macOS 的反斜線是合法檔名字元，不是分隔符

`"$HOME\.claude\skills\x"` 在 mac 上**不會**報「路徑格式錯誤」。它組出來的是一個名字裡真的帶反斜線的單層項目，於是 `Test-Path` 回 `False`，技能只會說「找不到腳本」——失敗原因指向完全錯誤的方向。

這條是上面〈路徑原則〉那句「不要出現字面 `\`」的後果說明，也是最常被漏掉的一種：前面幾條路徑規則都要求開頭有磁碟機代號，**變數接反斜線那種組法完全不在它們的視野內**。2026-09-20 補進 `check-platform.py` 後一次掃出 7 個 repo、47 處。

正解一律 `Join-Path`：

```powershell
$skills = Join-Path $HOME '.claude/skills'      # 正斜線兩平台都吃
```

### 檢查器怎麼分辨「路徑」與「正則」

這條規則在 `check-platform.py` 裡分成兩段，因為「變數接反斜線」與「整串都是字面」要用不同的方式才抓得準：

| 形態 | 例子 | 怎麼抓 |
|------|------|--------|
| 變數接反斜線 | `"$GEOM\render.py"` | `$變數` 當錨點，任何語言都掃 |
| 整串字面 | `'skills\clasp-setup'` | **只在 PowerShell 語境**掃，見下 |

第二種為什麼要限定語境：**兩種語言對反斜線的語意相反**。PowerShell 的跳脫字元是反引號，所以字串裡的反斜線幾乎必然是路徑分隔符；Python 剛好相反（`\n`、`\d`、`\s` 滿天飛）。混在一起掃必然誤報——實測「任何語言都掃」的版本會把 `re.split(r"\r?\n\r?\n")` 與 JSON 裡的 `"2 天前是\n4×(-2)"` 全算成路徑。

四層過濾，缺一層誤報就爆掉：

1. 只看 `.ps1`／`.psm1`，以及 `.md` 裡**標了 `powershell`** 的圍籬（沒標語言的圍籬一律不算——寧可漏，不要誤報）
2. 排除同一行有 `-match`／`-replace`／`-split`／`[regex]` 的（PowerShell 裡唯一會把字串當正則吃的場合）
3. 反斜線後面要接**兩個**路徑字元（首字可為點，因為 `.claude` 這種隱藏目錄很常見）
4. 排除含字元類別 `[...]` 或量詞 `{n,m}` 的行——補第 2 層漏掉的「正則存進變數、運算子不在同一行」

已知的兩個代價，都是**漏報不是誤報**：反斜線後緊接 `<佔位符>` 的寫法抓不到；非 PowerShell 檔裡的字面路徑也抓不到。

⚠️ **登錄檔路徑（`HKLM:` 那串）與磁碟機字母的範例路徑不要自動轉**。前者根本不是檔案路徑，正斜線是錯的；後者只會半轉成前半反斜線後半正斜線的四不像。這兩類用行內 `platform-ok:` 標記。
