...
5. `git status --short` 看變動 → 擬**繁體中文** commit 訊息（標題：動詞＋對象；正文 3-5 條 bullet 描述變動＋為什麼）→ **給使用者過目，點頭再 commit**
   ...
   **🚧 刪除閘門（擬訊息之前先過這關）**：`git status --short` 裡凡是 `D`（刪除），**逐條**自問
   「這是這個 session 刻意刪的嗎？」——**不是就停下來問使用者，不可放行**。
   - 刪除項要**單獨列一段**給使用者看，不可混在變動清單裡跟著批次點頭。清單一長，`D` 最容易被滑過去
   - `D AGENTS.md`／`D CLAUDE.md` 幾乎一定**不是你刪的**，而是雲端硬碟的純大小寫改名殘留
     （見 `AGENTS.md`〈工作約定〉那條）。正確處理是 `git checkout -- <檔名>` 還原、清掉 `(1)` 副本，
     **不是**把刪除 commit 出去——一旦推上去，藍圖就從 repo 消失，而且是在所有電腦上消失
   - 同理適用任何「我不記得刪過」的檔案。**還原的成本是一行指令，推錯的成本是回頭 revert 加重推**
...
- ❌ 把沒逐條確認過的刪除（`D`）commit 出去——尤其 `D AGENTS.md`／`D CLAUDE.md`，那通常是雲端硬碟同步殘留、不是你刪的（見步驟 5 的刪除閘門）
